...
.delete(authenticate.authenticateToken,(req,res) => {
    ...
    .then((discussion)=>{
        if(discussion.email == req.body.email){
            Discussion.findByIdAndDelete(req.body.discussionId)
            .then((discussion)=>{
                if(discussion != null){
                    res.statusCode = 200;
                    res.setHeader('Content-Type', 'application/json');
...
.post(authenticate.authenticateToken,(req,res) =>{
 ...
 .put(authenticate.authenticateToken, (req, res) => {
    Discussion.findById(req.body.discussionId)
        .then((discussion) => {
            if (discussion != null && discussion.comments.id(req.body.commentId) != null) {
                if (req.body.email == discussion.comments.id(req.body.commentId).email) {
                        discussion.comments.id(req.body.commentId).comment = req.body.comment;
                        discussion.save()
                        .then((discussion) => {
                            res.statusCode = 200;
                            res.setHeader('Content-Type', 'application/json');
